...
void outBinFile(string namebf) {
    ...
    cout << "—одержание бинарного файла: \n";
    student s;
...
//функци€ ищет запись с заданным ключом и при успешном поиске возвращает пор€дковый номер в записи в файле
int searchKeyBinFile(string namebf, int key) {
    ifstream bf;
    bf.open(namebf, ios::binary);
    if (!bf) return -2; //не открылс€
    student s;
    int numstudent = 0;
    bf.read((char*)&s, sizeof(student));
    while (!bf.eof()) {
...
int main()
{
    ...
    outBinFile("Data.dat");
    cout << "¬ведите номер группы дл€ перевода студентов: ";
    int newGroup;
    cin >> newGroup;
